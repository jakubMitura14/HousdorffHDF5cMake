...
template <typename T>
at::Tensor getHausdorffDistance_CUDA_FullResList_local(
    at::Tensor goldStandard,
    at::Tensor algoOutput,
    int WIDTH,
    int HEIGHT,
    ...
    ForBoolKernelArgs<T> fbArgs =
        executeHausdoff(forFullBoolPrepArgs, WIDTH, HEIGHT, DEPTH, occData, stream1, false, robustnessPercent, true);
...
at::Tensor getHausdorffDistance_CUDA_FullResList(
    ...
    at::Tensor res;
    AT_DISPATCH_ALL_TYPESWithBool(
        goldStandard.type(), "getHausdorffDistance_CUDA_FullResList", ([&] {
            res = getHausdorffDistance_CUDA_FullResList_local<scalar_t>(
                goldStandard, algoOutput, WIDTH, HEIGHT, DEPTH, robustnessPercent, compare_values);
            }));
